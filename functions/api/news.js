...
export async function onRequest(context) {
  ...
  if (method === 'GET') {
    ...
    let rows;
    if (cat) {
      rows = await env.DB.prepare("SELECT id,title,summary,url,source,category,published_at,image FROM news WHERE status='published' AND category=? ORDER BY published_at DESC LIMIT ?").bind(cat, limit).all();
    } else {
      rows = await env.DB.prepare("SELECT id,title,summary,url,source,category,published_at,image FROM news WHERE status='published' ORDER BY published_at DESC LIMIT ?").bind(limit).all();
    }
