// Shared product-import helpers used by both admin and member bulk import.
// Keeps the 4-channel logic (AI text parse, link adapters, commit + image re-host)
// in one place so behaviour stays identical for admins and VIP suppliers.
...
export const UA = 'Mozilla/5.0 (compatible; NexAmuseBot/1.0; +https://nexamuse.pages.dev)';
...
// SSRF guard: only public http(s) endpoints may be pulled.
export function urlGuard(raw) {
  let u;
  ...
  if (h === '::1' || h.startsWith('fd') || h.startsWith('fc') || h.startsWith('fe80')) {
    return 'Blocked host';
  }
...
export async function llmJson(env, systemPrompt, userPrompt) {
...
export async function tryShopify(u) {
...
export async function tryWoo(u) {
  const origin = u.origin;
...
export function findProducts(obj, out, depth) {
  if (!out) out = [];
...
export async function tryGeneric(u) {
...
export async function parseText(env, text) {
  if (!text) return { ok: false, error: 'Text is required' };
  if (text.length > 12000) return { ok: false, error: 'Text is too long (max 12000 characters)' };
  const system = [
...
export async function fetchLink(rawUrl) {
  if (!rawUrl) return { ok: false, supported: false, reason: 'URL is required' };
  const blocked = urlGuard(rawUrl);
  if (blocked) return { ok: false, supported: false, reason: blocked };
  let u;
  try { u = new URL(rawUrl); } catch (e) { return { ok: false, supported: false, reason: 'Invalid URL' }; }

  const attempts = [];
  try {
    const prods = await tryShopify(u);
    if (prods && prods.length) return { ok: true, supported: true, source: 'shopify', products: prods };
    attempts.push('shopify: no products');
  } catch (e) { attempts.push('shopify: ' + e.message); }
  try {
    const prods = await tryWoo(u);
    if (prods && prods.length) return { ok: true, supported: true, source: 'woocommerce', products: prods };
  ...
  try {
    ...
    if (res && res.products.length) {
      return { ok: true, supported: true, source: res.source, products: res.products };
    }
...
// Insert products for a company. `status` controls visibility ('pending' for admin
// assisted imports, 'active' for a supplier managing their own storefront).
export async function commitProducts(products, env, companyId, ownerId, status, uploadToTutu) {
  const list = Array.isArray(products) ? products : [];
  if (!list.length) return { ok: false, error: 'No products to import' };

  const company = await env.DB.prepare('SELECT id, owner_id FROM companies WHERE id=?').bind(companyId).first();
  if (!company) return { ok: false, error: 'Company not found', code: 404 };
  const finalOwner = company.owner_id || ownerId;
